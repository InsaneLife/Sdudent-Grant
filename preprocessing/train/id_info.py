...
def set_id_how():
    ...
    id_how = pd.DataFrame(card_train.groupby(['id', 'how'])['consume'].count())
    id_how['a_sum'] = card_train.groupby(['id', 'how'])['amount'].sum()
    id_how['a_avg'] = card_train.groupby(['id', 'how'])['amount'].mean()
    id_how['a_max'] = card_train.groupby(['id', 'how'])['amount'].max()
    id_how.to_csv('D:/Competition/student_grant/input/train/id_how.csv', index=True)


def card_how():
    sql = 'SELECT id FROM id_how_t group by id '
    id_info = pd.read_sql(sql, conn, index_col=None)

    sql = "SELECT i.* FROM id_how_t i WHERE i.how = '食堂'"
    s_st = pd.read_sql(sql, conn, index_col=None)
    del s_st['how']
    id_info = pd.merge(id_info, s_st, how='left', on='id')

    sql = "SELECT * FROM id_how_t i WHERE i.how = '开水'"
    s_ks = pd.read_sql(sql, conn)
    del s_ks['how']
    id_info = pd.merge(id_info, s_ks, how='left', on='id')

    sql = "SELECT * FROM id_how_t i WHERE i.how = '淋浴'"
    s_ks = pd.read_sql(sql, conn)
    del s_ks['how']
    id_info = pd.merge(id_info, s_ks, how='left', on='id')

    sql = "SELECT * FROM id_how_t i WHERE i.how = '校车'"
    s_ks = pd.read_sql(sql, conn)
    del s_ks['how']
    id_info = pd.merge(id_info, s_ks, how='left', on='id')

    sql = "SELECT * FROM id_how_t i WHERE i.how = '超市'"
    s_ks = pd.read_sql(sql, conn)
    del s_ks['how']
    id_info = pd.merge(id_info, s_ks, how='left', on='id')

    sql = "SELECT * FROM id_how_t i WHERE i.how = '洗衣房'"
    s_ks = pd.read_sql(sql, conn)
    del s_ks['how']
    id_info = pd.merge(id_info, s_ks, how='left', on='id')

    sql = "SELECT * FROM id_how_t i WHERE i.how = '图书馆'"
    s_ks = pd.read_sql(sql, conn)
    del s_ks['how']
    id_info = pd.merge(id_info, s_ks, how='left', on='id')

    sql = "SELECT * FROM id_how_t i WHERE i.how = '文印中心'"
    s_ks = pd.read_sql(sql, conn)
    del s_ks['how']
    id_info = pd.merge(id_info, s_ks, how='left', on='id')
    #
    # sql = "SELECT * FROM id_how i WHERE i.how = '教务处'"
    # s_ks = pd.read_sql(sql, conn)
    # del s_ks['how']
    # id_info = pd.merge(id_info, s_ks, how='left', on='id')
    #
    # sql = "SELECT * FROM id_how i WHERE i.how = '校医院'"
    # s_ks = pd.read_sql(sql, conn)
    # del s_ks['how']
    # id_info = pd.merge(id_info, s_ks, how='left', on='id')
    ...
    # print id_info
    id_info.to_csv('D:/Competition/student_grant/input/train/id_info.csv', index=False)
    return id_info
